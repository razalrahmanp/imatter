...
// ── Tag patterns ───────────────────────────────────────────────────────────────
const START_RE = /<!--\s*SDLC:start\s+(.*?)\s*-->/;
const END_RE = /<!--\s*SDLC:end\s+(.*?)\s*-->/;
const PLACEHOLDER_RE = /<!--\s*SDLC:placeholder\s+(.*?)\s*-->/;
const VERSION_RE = /<!--\s*SDLC:version\s+"?([\d.]+)"?\s*-->/;
...
// ── Main parser ───────────────────────────────────────────────────────────────
export function parseRegions(docContent) {
    ...
    function warnUnknownAttrs(attrs, known, line, id) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (startMatch) {
            ...
            if (type === "framework")
                warnUnknownAttrs(attrs, KNOWN_FRAMEWORK_ATTRS, lineNum, id);
            else if (type === "user-override")
                warnUnknownAttrs(attrs, KNOWN_USER_OVERRIDE_ATTRS, lineNum, id);
            else
                warnUnknownAttrs(attrs, KNOWN_USER_ATTRS, lineNum, id);
        ...
        if (endMatch) {
            ...
            const dirty = frame.type === "framework" && !!recordedHash && contentHash !== recordedHash;
            if (dirty) {
                warnings.push({
                    code: "HASH_MISMATCH",
            ...
            if (newCurrent) {
                for (const cl of frame.contentLines)
                    newCurrent.contentLines.push(cl);
        ...
        if (r.type === "user-override" && r.overrides && r.originalHash) {
            ...
            if (fw?.hash && fw.hash !== r.originalHash) {
                ...
                warnings.push({
                    code: "OVERRIDE_STALE",
                    line: r.startLine,
                    message: `Override "${r.id}" was created against framework hash ${r.originalHash}, ` +
 ...
 */
export function serializeRegions(originalLines, regions) {
    ...
    const out = [];
    const skip = new Set(); // lines consumed by a region
    for (let i = 0; i < originalLines.length; i++) {
        ...
        if (region) {
            ...
            for (let j = lineNum + 1; j <= region.endLine; j++)
                skip.add(j);
