...
export async function runMigrations(opts) {
    ...
    for (const script of applicable) {
        ...
        const newErrors = check.errors.filter((e) => !parsed.errors.some((pe) => pe.message === e.message));
        if (newErrors.length > 0) {
            throw new Error(`Migration to ${script.to} introduced parse errors — aborting to protect the document:\n` +
                newErrors.map((e) => `  ${e.message}`).join("\n") +
                (backupPath ? `\nBackup preserved at: ${backupPath}` : ""));
        }
        if (check.errors.length > 0) {
            allWarnings.push(`Pre-existing parse errors remain after ${script.to} migration:\n` +
