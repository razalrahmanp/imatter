...
function parseArgs(argv) {
    ...
    if (has("--apply"))
        mode = "apply";
    else if (has("--rollback") || argv.some((a) => a.startsWith("--rollback=")))
        mode = "rollback";
...
async function loadMigrationScripts() {
    ...
    if (!existsSync(migrationsDir))
        return [];
    const files = readdirSync(migrationsDir).filter((f) => /\.(js|ts)$/.test(f) && !f.endsWith(".d.ts"));
    const scripts = [];
...
// ── Version detection ──────────────────────────────────────────────────────────
function getClientVersion(sdlcContent, projectRoot) {
...
function detectUnauthorizedEdits(sdlcContent) {
    const parsed = parseRegions(sdlcContent);
    const out = [];
...
// ── Rollback ──────────────────────────────────────────────────────────────────
function rollback(projectRoot, timestamp, format) {
    ...
    if (target.ageMs > MAX_AGE_MS) {
        const msg = `Backup ${target.timestamp} is older than 30 days (${formatAge(target.ageMs)}) — ` +
    ...
    if (format === "json") {
        ...
        process.stdout.write(`Rolled back to ${target.timestamp} (${formatAge(target.ageMs)})\n` +
