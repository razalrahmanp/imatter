...
<!-- SDLC:end id="outer" -->`;
    const result = parseRegions(input);
    assert.ok(result.errors.some((e) => e.code === "NESTED_FRAMEWORK"), "must surface NESTED_FRAMEWORK error");
});
...
<!-- SDLC:end id="x" -->`;
    ...
    assert.ok(result.warnings.some((w) => w.code === "HASH_MISMATCH"), "must emit HASH_MISMATCH warning");
});
...
test("template-generator: SECTION_MAP must include all base sections", () => {
    ...
    for (const heading of required) {
        assert.ok(heading in SECTION_MAP, `SECTION_MAP missing required heading: "${heading}". ` +
...
<!-- SDLC:end id="rr" -->`;
    ...
    const { newContent, applied } = applyResolutions(input, [{ regionId: "rr", action: "discard" }], registry);
...
<!-- SDLC:end id="rs" -->`;
    ...
    const { newContent, applied } = applyResolutions(input, [{ regionId: "rs", action: "override" }], registry);
    assert.equal(applied.length, 1);
...
test("plugin: at least one migration script exists and exports `migration`", async () => {
    ...
    for (const f of files) {
        ...
        assert.match(content, /export\s+const\s+migration\s*[:=]/, `migration ${f} must export a const named "migration"`);
    }
});
