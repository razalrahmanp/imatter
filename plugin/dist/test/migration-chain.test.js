...
function makeTempDir() {
    const dir = join(tmpdir(), `sdlc-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
async function migrateFixture(dir, dryRun = false) {
...
test("runMigrations 1.0.0→1.1.0: inserts version marker and reports step", async () => {
    ...
    try {
        ...
        assert.ok(result.finalContent.includes('<!-- SDLC:version "1.1.0" -->'), "finalContent must contain the version marker");
...
test("runMigrations 1.0.0→1.1.0: parseRegions sees frameworkVersion 1.1.0", async () => {
    ...
    try {
        ...
        assert.equal(parsed.frameworkVersion, "1.1.0", "parseRegions must extract frameworkVersion from the version marker");
    }
    finally {
        cleanup(dir);
    }
...
test("runMigrations 1.0.0→1.1.0: version marker is placed immediately after H1", async () => {
    ...
    try {
        ...
        assert.equal(lines[h1Index + 1], '<!-- SDLC:version "1.1.0" -->', "version marker must be the line immediately after the H1");
    }
    finally {
        cleanup(dir);
    }
...
test("runMigrations: creates backup before mutating the file", async () => {
    ...
    try {
        ...
        assert.ok(!backupContent.includes('<!-- SDLC:version "1.1.0" -->'), "backup must contain original v1.0.0 content without the version marker");
    }
    finally {
        cleanup(dir);
    }
...
test("runMigrations: writes migrated content to disk", async () => {
    ...
    try {
        ...
        assert.ok(onDisk.includes('<!-- SDLC:version "1.1.0" -->'), "version marker must be present in the on-disk file after migration");
    }
    finally {
        cleanup(dir);
    }
...
test("runMigrations dry-run: does not write to disk, backupPath is null", async () => {
    ...
    try {
        ...
        const onDisk = readFileSync(join(dir, "SDLC_VALIDATION.md"), "utf-8");
        assert.ok(!onDisk.includes('<!-- SDLC:version "1.1.0" -->'), "dry-run must not write to the file on disk");
        assert.ok(result.finalContent.includes('<!-- SDLC:version "1.1.0" -->'), "dry-run must still return migrated content in-memory via finalContent");
    }
    finally {
        cleanup(dir);
    }
...
//# sourceMappingURL=migration-chain.test.js.map