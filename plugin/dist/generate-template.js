...
function parseArgs(argv) {
    const version = argv.find((a) => a.startsWith("--version="))?.split("=")[1] ?? "1.0.0";
...
function main() {
    ...
    if (!existsSync(sourcePath)) {
        ...
        process.stderr.write(`Note: No .source.md found — using ${fallback} as source (bootstrapping).\n` +
    ...
    if (unknownSections.length > 0) {
        process.stderr.write(`Warning: ${unknownSections.length} section(s) not in SECTION_MAP (emitted untagged):\n` +
