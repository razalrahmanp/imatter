...
export function createServer() {
    ...
    server.tool("sdlc_verify_history", "Verify the cryptographic integrity of SDLC gate history. Checks HMAC signatures on every " +
        "history entry, the cursor, and the top-level state file. Call when you suspect the state " +
        "file may have been edited outside the tool or to audit gate records before a compliance review.", { project_root: z.string().optional() }, async ({ project_root }) => {
        try {
            const root = resolveProjectRoot(project_root);
            ...
            const entryLines = report.entries.length === 0
                ? "  (no history entries)"
                : report.entries
                    .map((e) => `  Stage ${e.stage} (${e.name}): gate=${e.gate} score=${e.score} ` +
                    `hmac=${e.hmac} doc=${e.docHash} cleared=${e.cleared_at}`)
                    .join("\n");
            const errorBlock = report.errors.length > 0
                ? `\n\nERRORS:\n${report.errors.map((e) => `  ⛔ ${e}`).join("\n")}`
                : "";
            const warnBlock = report.warnings.length > 0
    ...
    server.tool("sdlc_trace_requirements", "Trace a requirement identifier through SDLC_VALIDATION.md and gate history. Shows every " +
        ...
        try {
            ...
            if (stageMatches.length > 0) {
                const passed = stageMatches.filter((m) => m.gateStatus === "PASSED" || m.gateStatus === "ONGOING");
                const unpassed = stageMatches.filter((m) => m.gateStatus !== "PASSED" && m.gateStatus !== "ONGOING");
                lines.push(`Coverage: ${stageMatches.length} stage(s) mention this requirement — ` +
                    `${passed.length} PASSED/ONGOING, ${unpassed.length} not yet passed`);
                if (unpassed.length > 0) {
                    lines.push(`Not yet passed: ${unpassed.map((m) => `Stage ${m.stageNumber}`).join(", ")}`);
                }
            }
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
