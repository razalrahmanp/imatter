...
async function main() {
    ...
    if (parsed.regions.length > 0 && !force) {
        process.stdout.write(`${sdlcPath} already has ${parsed.regions.length} region(s). Use --force to re-tag.\n`);
        process.exit(0);
    }
    ...
    if (matchedCount === 0) {
        process.stderr.write(`Warning: no SECTION_MAP headings found in ${sdlcPath}.\n` +
