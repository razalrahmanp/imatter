...
function parseArgs(argv) {
    const format = argv.find((a) => a.startsWith("--format="))?.split("=")[1] === "json"
        ? "json"
        : "text";
...
function loadRegistry(registryPath) {
    ...
    throw new Error(`No registry.json found. Run 'npx tsx src/generate-template.ts' to generate it.`);
}
// ── Core check ────────────────────────────────────────────────────────────────
function runCheck(sdlcContent, registry, frameworkVersion) {
    ...
    for (const warn of parsed.warnings) {
        const severity = warn.code === "HASH_MISMATCH" ? "error" : "warning";
        reports.push({
            id: warn.id ?? "(unknown)",
            severity,
            code: warn.code,
    ...
    const docFrameworkIds = new Set(parsed.regions
    ...
    for (const [id, canonical] of registry) {
        if (!docFrameworkIds.has(id)) {
            reports.push({
                ...
                detail: `This region was introduced in framework v${canonical.since}. ` +
    ...
    if (!parsed.isFullyTagged && parsed.untagged.length > 0) {
        reports.push({
            ...
            detail: `This document has not been fully migrated to the region-marker format. ` +
...
function renderText(out) {
    ...
    lines.push(`\n${"─".repeat(60)}`, `OK: ${out.summary.ok}  WARNINGS: ${out.summary.warnings}  ERRORS: ${out.summary.errors}`, out.is_fully_tagged
