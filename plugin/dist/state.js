...
export function readState(projectRoot) {
    ...
    if (!existsSync(path))
        throw new Error(`No .sdlc-state.json at ${path}. Run sdlc_state_create first.`);
...
export function writeState(projectRoot, state) {
    const path = statePath(projectRoot);
    const tmp = path + ".tmp";
    const signed = signState(state, projectRoot);
    writeFileSync(tmp, JSON.stringify(signed, null, 2), "utf-8");
    renameSync(tmp, path);
}
export function ensureSessionDir(projectRoot) {
...
export function extractExport(frontmatter, key) {
    const exports = frontmatter["exports"];
    return exports?.[key];
}
// ── Quality scoring ───────────────────────────────────────────────────────────
function computeScore(memory, criteria) {
    let totalWeight = 0;
    let earnedWeight = 0;
    for (const criterion of criteria) {
        const finding = memory[criterion.ns];
        if (!finding)
            continue;
...
export function runGateSynthesis(config) {
    ...
    const hjEntry = Object.entries(memory).find(([, f]) => f !== null && f.requires_human_judgment);
    if (hjEntry) {
        return {
    ...
    for (const criterion of criteria) {
        const finding = memory[criterion.ns];
        if (!finding) {
            blockingFailed.push(`${criterion.ns}: no finding`);
            continue;
        }
        ...
        let met = true;
        if (criterion.must === "pass" && finding.status !== "pass")
            met = false;
        else if (criterion.must === "pass_or_acknowledge" && !["pass", "acknowledged"].includes(finding.status))
            met = false;
    ...
    for (const [ns, finding] of Object.entries(memory)) {
        ...
        for (const artifact of finding.artifacts) {
            ...
            if (existing && existing.status !== finding.status) {
                conflicts.push(`Conflict on "${artifact}": ${existing.ns}=${existing.status} vs ${ns}=${finding.status}`);
            }
            artifactMap.set(artifact, { ns, status: finding.status });
        }
    }
...
// ── Task checkpoint I/O ───────────────────────────────────────────────────────
export function taskDir(projectRoot) {
    const dir = join(projectRoot, ".sdlc-tasks");
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
    return dir;
}
export function taskPath(projectRoot, taskId) {
...
export function diagnoseError(rawOutput, filePath) {
...
function findFileRef(lines, around, defaultFile) {
    ...
    for (const l of range) {
        ...
        if (m)
            return { file: m[1], line: parseInt(m[2]), col: m[3] ? parseInt(m[3]) : undefined };
    }
    return { file: defaultFile };
}
function extractExcerpt(lines, center, radius) {
    return lines
