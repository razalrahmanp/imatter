...
import type { MigrationScript, MigrationResult, MigrationContext } from "../migration.js";
...
export const migration: MigrationScript = {
  ...
  apply(ctx: MigrationContext): MigrationResult {
    ...
    const { tagged, unknownSections } = generateTaggedTemplate(
      ctx.lines.join("\n"),
      ctx.toVersion,
    );
    ...
    if (unknownSections.length > 0) {
      warnings.push(
        `${unknownSections.length} section(s) were not recognized and were left untagged:\n` +
    ...
    if (dirtyCount > 0) {
      warnings.push(
        `${dirtyCount} framework region(s) have content that differs from the v1.1.0 canonical template. ` +
