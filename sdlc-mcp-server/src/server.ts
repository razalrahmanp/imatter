...
export function createServer(): McpServer {
  ...
  server.tool(
    "load_sdlc_context",
    "Load the full SDLC_VALIDATION.md into Claude's context. " +
      "Called automatically by the SessionStart hook — Claude is bound by the rules it contains.",
    ...
    async ({ project_root }) => {
      try {
        ...
        const blocked = statuses.filter(
          (s) => s.stage <= 10 && s.status !== "PASSED" && s.status !== "ONGOING"
        );
        const gateNote =
          blocked.length > 0
          ...
          content: [
            {
              type: "text",
              text:
                `SDLC_VALIDATION.md loaded from ${sdlcPath}\n` +
  ...
  server.tool(
    "sdlc_session_brief",
    "Single-call session-start summary. Returns cursor, gates cleared, flagged stages, " +
      "last Section-18 entry, and active stage gate status in one read. " +
      "Use this in place of separately reading .sdlc-state.json and Section 18.",
    { project_root: z.string().optional() },
    async ({ project_root }) => {
      try {
        const root = resolveProjectRoot(project_root);
        ...
        try {
          ...
          if (rows.length > 0) lastSession = rows[rows.length - 1];
        } catch {
          // Section 18 may not exist yet
        }

        const lines: string[] = [`SDLC session brief — ${sdlcPath}`, ``];
        ...
        if (state) {
          ...
          lines.push(
            `Flagged stages: ${state.flagged.length ? state.flagged.join(", ") : "none"}`,
          );
          const activeGate = gateStatuses.find((g) => g.stage === state.cursor.stage);
          if (activeGate) {
            lines.push(
              `Active stage gate: Stage ${activeGate.stage} (${activeGate.name}) — \`${activeGate.status}\``,
            );
          }
        } else {
          lines.push(`No .sdlc-state.json — falling back to SDLC doc quick reference.`);
          const passed = gateStatuses.filter(
            (g) => g.status === "PASSED" || g.status === "ONGOING",
          );
          lines.push(
            `Gates passed/ongoing: ${passed.length ? passed.map((g) => `Stage ${g.stage}`).join(", ") : "none"}`,
          );
          if (stateError) lines.push(`(state read error: ${stateError})`);
        }
  ...
  server.tool(
      ...
      "Use log_level=minimal for sensitive projects to avoid committing implementation details to git.",
    {
      ...
      log_level: z
        ...
        .optional()
        .describe(
          "minimal: omit work_done details (logs 'Session completed'); " +
    ...
    async ({ work_done, gates_changed, next_step, log_level, project_root }) => {
      try {
        ...
        let rowWorkDone: string;
        let rowNextStep: string;
  ...
  server.tool(
    "init_project",
    "Copy the bundled SDLC_VALIDATION.md template into the project root. " +
      "Run this once when starting a new project — never overwrites an existing file.",
    { project_root: z.string().optional() },
    async ({ project_root }) => {
      const root = resolveProjectRoot(project_root);
        ...
        content: [
          {
            type: "text",
            text:
              `SDLC_VALIDATION.md created at ${dest}\n\n` +
  ...
  server.tool(
    "sdlc_skills_fetch",
    "Fetch a skill's pattern summary from the registry. " +
      ...
      "Use task_type='list' to enumerate all available skills with their layer and tags.",
    {
      task_type: z
        .string()
        .describe(
          "Skill id (file name without extension), e.g. 'lambda-handler', 'api-endpoint-design'. " +
    ...
    async ({ task_type, tags, module: mod, project_root }) => {
      try {
        ...
        // ── Load .sdlc-stack.json for registry resolution order ───────────────
        ...
        const stackConfig: StackConfig = fs.existsSync(stackConfigPath)
          ? (JSON.parse(fs.readFileSync(stackConfigPath, "utf-8")) as StackConfig)
          : {};

        // Ordered search paths — highest priority first
        const searchPaths: Array<{ layer: string; dir: string }> = [
        ...
        function extractSummary(content: string): string {
          ...
          for (const line of lines) {
            if (line.startsWith("## ")) {
              count++;
              if (count === 1) { inSummary = true; out.push(line); continue; }
              if (count === 2) break;
            }
        ...
        function parseFm(content: string): string | null {
        ...
        let resolvedLayer: string | null = null;
        ...
        if (!resolvedPath) {
          ...
          return {
            content: [{
              type: "text",
              text:
                `Skill "${task_type}" not found in registry.\n` +
                (all.size > 0
        ...
        if (tags?.length && fm) {
          const tagsLine = fm.match(/^tags:\s*(\[.+\])/m)?.[1];
          const skillTags: string[] = tagsLine ? (JSON.parse(tagsLine) as string[]) : [];
  ...
  server.tool(
    "sdlc_state_create",
    "Initialize .sdlc-state.json for a project. Run once per project before any stage work. " +
      "Reads the Quick Reference table in SDLC_VALIDATION.md to discover all stages and sets the " +
      "cursor to the first NOT STARTED stage. Never overwrites an existing state file.",
    { project_root: z.string().optional() },
    async ({ project_root }) => {
      try {
        const root = resolveProjectRoot(project_root);
  ...
  server.tool(
    "sdlc_init",
    "Assemble the session context for the current (or specified) stage. " +
    ...
    async ({ project_root, stage }) => {
      try {
        ...
        if (!lockResult.acquired) {
          ...
          return {
            content: [{
              type: "text",
              text:
                `LOCK CONFLICT — another session holds the SDLC state lock.\n` +
        ...
        try { regenerateQuickReference(state, state.sdlc_file); } catch { /* best effort */ }
        ...
        if (state.flagged.includes(targetStage)) {
          return {
            content: [{
              type: "text",
              text:
                `Stage ${targetStage} is in the flagged queue — it has failed ${state.cursor.fail_count} times.\n` +
        ...
        const historySummaries = state.history.map((h) =>
          `Stage ${h.stage} (${h.name}): ${h.gate} on ${h.cleared_at}. ${h.summary}`
        ).join("\n");
        ...
        if (stageConfig?.sdlc_heading) {
          ...
          const headingMatch = sdlcContent.match(
            new RegExp(`## (?:\\d+\\. )?Stage ${targetStage}[^\\n]*`)
          );
          if (headingMatch) {
            const heading = headingMatch[0].replace(/^## /, "");
            try { sdlcSection = getSdlcSection(sdlcContent, heading); } catch { /* leave default */ }
          }
        }
        ...
        return {
          content: [{
            type: "text",
            text:
              `sdlc_init — Stage ${targetStage} session context${lockNote}${versionNote}\n` +
              `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
              `${configNote}\n\n` +
              `Full payload (JSON):\n` +
  ...
  server.tool(
    "sdlc_agent_write",
    "Record a sub-agent's findings into the current stage's memory namespace. " +
      ...
      try {
        ...
        return {
          content: [{
            type: "text",
            text:
              `Finding recorded: Stage ${targetStage} / ns="${ns}" / status=${status}\n` +
              (remaining.length > 0
  ...
  server.tool(
    "sdlc_gate_run",
    "Synthesize sub-agent findings and decide PASS or FAIL for the current stage gate. " +
    ...
    async ({ stage_summary, doc_path, stage, project_root }) => {
      try {
        ...
        const stageConfig = state.stages[stageKey] as StageConfig;
        ...
        if (result.verdict === "HUMAN_JUDGMENT") {
          ...
          return {
            content: [{
              type: "text",
              text:
                `HUMAN JUDGMENT REQUIRED — Stage ${targetStage}\n` +
                `${result.reason}\n\n` +
        ...
        if (result.needs_arbitration) {
          return {
            content: [{
              type: "text",
              text:
                `Conflict arbitration needed before gate can decide.\n` +
        ...
        if (result.verdict === "PASS" || result.verdict === "CONCERNS") {
          ...
          for (const finding of Object.values(stageConfig.memory)) {
            if (finding) allFlags.push(...finding.flags);
          }
          ...
          let docSha256: string | null = null;
          try {
            if (fs.existsSync(docAbsPath)) docSha256 = fileHash(docAbsPath);
          ...
          if (reviewer) {
            ...
            return {
              content: [{
                type: "text",
                text:
                  `GATE ${gateLabel} (score: ${result.score}/100) — Stage ${targetStage} (${stageConfig.name})\n` +
          ...
          state.history.push({
            ...
            doc_sha256: docSha256,
            exports: Object.keys(
              (parseFrontmatter(docAbsPath)["exports"] as Record<string, unknown> | undefined) ?? {}
            ),
            score: result.score,
            concerns: result.concerns,
          ...
          return {
            content: [{
              type: "text",
              text:
                `GATE ${gateLabel} — Stage ${targetStage} (${stageConfig.name})\n` +
        ...
        if (failCount >= 2) {
          state.cursor.status = "awaiting_review";
          if (!state.flagged.includes(targetStage)) state.flagged.push(targetStage);
          writeState(root, state);
          return {
            content: [{
              type: "text",
              text:
                `GATE FAILED (${failCount} failures) — Stage ${targetStage} added to flagged[] queue.\n` +
        ...
        return {
          content: [{
            type: "text",
            text:
              `GATE FAILED (${failCount}/2 before human review) — Stage ${targetStage}\n` +
              `Failed criteria:\n` +
              result.failed_criteria?.map((c) => `  • ${c}`).join("\n") +
  ...
  server.tool(
    "sdlc_gate_waive",
    "Record an explicit waiver for a gate criterion, then mark the stage WAIVED and advance the cursor. " +
    ...
    async ({ criterion_ns, reason, approved_by, stage_summary, doc_path, stage, project_root }) => {
      try {
        ...
        const date = new Date().toISOString().slice(0, 10);
        const waiver: Waiver = { stage: targetStage, criterion_ns, reason, approved_by, waived_at: date };
        ...
        for (const key of Object.keys(stageConfig.memory)) stageConfig.memory[key] = null;
        writeState(root, state);
        ...
        return {
          content: [{
            type: "text",
            text:
              `GATE WAIVED — Stage ${targetStage} (${stageConfig.name})\n` +
  ...
  server.tool(
    "sdlc_release_lock",
    "Release the session lock acquired by sdlc_init. " +
      "Call at end of session or if sdlc_init refuses due to a stale lock you own. " +
      "Does nothing if no lock file exists.",
    { project_root: z.string().optional() },
    async ({ project_root }) => {
      try {
        const root = resolveProjectRoot(project_root);
  ...
  server.tool(
    "sdlc_signoff",
    "Complete a gate transition that requires human reviewer sign-off. " +
    ...
    async ({ approved_by, stage, project_root }) => {
      try {
        ...
        const stageConfig = state.stages[String(targetStage)] as StageConfig;
        ...
        return {
          content: [{
            type: "text",
            text:
              `GATE ${pending.gate_verdict} — Stage ${targetStage} signed off by ${approved_by}\n` +
  ...
  server.tool(
    "sdlc_doctor",
    "Diagnose the health of the SDLC state: framework version match, integrity status, " +
      "evidence staleness, and pending issues. Run at session start when something seems wrong.",
    { project_root: z.string().optional() },
    async ({ project_root }) => {
      try {
        const root = resolveProjectRoot(project_root);

        const lines: string[] = ["── SDLC Doctor ──────────────────────────────────────────\n"];
        ...
        try {
          state = JSON.parse(fs.readFileSync(statePath, "utf-8")) as SdlcState;
          const { verifyState: verify } = await import("./integrity.js");
          const result = verify(state as unknown as Record<string, unknown>, root);
          if (!result.ok) {
            integrityOk = false;
  ...
  server.tool(
    ...
    "Flush writer state to .sdlc-tasks/{task_id}.json after each iteration and return a compact " +
    ...
    async ({ task_id, file_path, status, changes_made, next_action, current_error, error_lines, stage, project_root }) => {
      try {
        ...
        if (resolvedStage === undefined) {
          try { resolvedStage = readState(root).cursor.stage; } catch { resolvedStage = 0; }
        }
        ...
        const task: TaskCheckpoint = existing
          ? {
              ...existing,
        ...
        return {
          content: [{
            type: "text",
            text:
              `sdlc_task_checkpoint — ${task_id} / iteration ${iteration.n}\n` +
              `${statusLine}\n\n` +
  ...
  server.tool(
    "sdlc_error_diagnose",
    "Classify raw compiler/linter/test output into a structured error payload. " +
      ...
      try {
        ...
        if (task_id) {
          ...
          if (task && task.iterations.length > 0) {
            ...
            last.error_lines = errors.flatMap((e) =>
              e.line ? [`${e.file}:${e.line}`] : []
            );
            writeTask(root, task);
          }
        }
        ...
        return {
          content: [{
            type: "text",
            text:
              `sdlc_error_diagnose — ${errorCount} error(s), ${warnCount} warning(s)\n` +
              `File: ${file_path}\n` +
              `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
              formatted +
              `\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
