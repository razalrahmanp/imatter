...
import type { SdlcState, HistoryEntry } from "./state.js";
  ...
  return match[1]
    ...
    .map((row) => {
      ...
      return {
        ...
        status: rawStatus as GateStatus["status"],
...
export function getSdlcSection(content: string, heading: string): string {
  ...
  if (start === -1) throw new Error(`Section "## ${heading}" not found in SDLC file.`);
 ...
 */
export function regenerateQuickReference(state: SdlcState, sdlcPath: string): void {
  // Build stage → latest cleared entry map
  const clearedMap = new Map<number, HistoryEntry>();
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!inQrSection) {
      if (line.startsWith("## Quick Reference")) inQrSection = true;
      continue;
    }

    // Exit when next section starts
    if (line.startsWith("## ") && !line.startsWith("## Quick Reference")) break;

    // Advance past the separator row (|---|---|...|)
    if (!pastSeparator) {
      if (/^\|[-| :]+\|/.test(line)) pastSeparator = true;
      continue;
    }
    ...
    if (currentStatus.includes("ONGOING")) continue;
...
export function artifactInfo(
  artifactPath: string,
  projectRoot: string
): { exists: boolean; fullPath: string; lineCount?: number; isDir?: boolean } {
  const fullPath =
    artifactPath.match(/^([A-Za-z]:[/\\]|\/)/) ? artifactPath : join(projectRoot, artifactPath);

  if (!existsSync(fullPath)) return { exists: false, fullPath };

  const stat = statSync(fullPath);
  if (stat.isDirectory()) return { exists: true, fullPath, isDir: true };
