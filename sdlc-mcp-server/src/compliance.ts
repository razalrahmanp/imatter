...
export interface ComplianceStatus {
  ...
  available_skills: Record<string, string[]>; // module → list of skill ids
  out_of_scope_skills: string[];
  recommendations: string[];
}

export function readStackConfig(projectRoot: string): StackConfig {
  const p = join(projectRoot, ".sdlc-stack.json");
  if (!existsSync(p)) return {};
  try {
    return JSON.parse(readFileSync(p, "utf-8")) as StackConfig;
  } catch {
    return {};
  }
}
 ...
 */
export function listComplianceSkillsInPlugin(
...
export function computeComplianceStatus(
  ...
  if (declared.length === 0) {
    recommendations.push(
      "No compliance modules declared. Edit .sdlc-stack.json to add a 'compliance' array if any regimes apply (gdpr, hipaa, soc2, pci-dss, eu-ai-act, etc.)."
    );
  }
  if (unknownDeclared.length > 0) {
    recommendations.push(
      `Declared compliance modules not recognized: ${unknownDeclared.join(", ")}. ` +
  ...
  if (declaredKnown.some((m) => !allByModule[m])) {
    ...
    recommendations.push(
      `Declared module(s) without shipping skills in the plugin: ${missing.join(", ")}.`
    );
  }
  if (declaredKnown.length > 0 && outOfScope.length > 0) {
    recommendations.push(
      `${outOfScope.length} compliance skill(s) present in plugin but outside declared modules. ` +
...
export function formatComplianceStatus(s: ComplianceStatus): string {
  ...
  if (s.out_of_scope_skills.length > 0) {
    lines.push("");
    lines.push(
      `Out-of-scope compliance skills present in plugin (${s.out_of_scope_skills.length}):`
    );
